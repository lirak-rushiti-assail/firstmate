...
`bin/fm-dashboard.sh` consumes the same contract for its terminal panels by collecting one snapshot and passing it to the bearings projection through `FM_BEARINGS_SNAPSHOT_JSON`, so one dashboard refresh costs one remote-ledger collection and every panel describes the same instant.

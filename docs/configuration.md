...
FM_DASHBOARD_CACHE_TTL=300   # seconds a cached bin/fm-dashboard.sh fleet snapshot or Microsoft 365 widget stays fresh before a refresh re-collects it; a cached fleet snapshot is also re-collected when the FM_SNAPSHOT_* collection settings differ from the ones it was collected under; 0 disables reuse and --force-refresh bypasses it for one run

...
FM_DASHBOARD_CACHE_TTL=300   # seconds a cached bin/fm-dashboard.sh fleet snapshot stays fresh before a render re-collects it; it is also re-collected when the FM_SNAPSHOT_* collection settings differ from the ones it was collected under; 0 disables reuse and --force-refresh bypasses it for one run
FM_DASHBOARD_TODO=10         # fleet-wide queued rows (this home's structured queued backlog plus the queued rows of each structured secondmate home's captain-actionable inventory; that inventory's held in-flight rows render under In Progress instead) the bin/fm-dashboard.sh Todo column shows before disclosing the rest

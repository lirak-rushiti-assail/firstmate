...
FM_DASHBOARD_CACHE_TTL=300   # seconds a cached bin/fm-dashboard.sh fleet snapshot or Microsoft 365 widget stays fresh before a refresh re-collects it; 0 disables reuse and --force-refresh bypasses it for one run
FM_DASHBOARD_STATE_DIR=$FM_HOME/state/dashboard   # private dashboard cache and seen-marker directory
